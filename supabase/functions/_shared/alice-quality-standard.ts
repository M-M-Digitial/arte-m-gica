export const ALICE_QUALITY_STANDARD = {
  ...
  layout: {
    ...
    milkNamePlateWidth: { min: 30, target: 38, max: 44 },
    milkPersonalizationFaces: 2,
    ageToNameFontRatio: { min: 0.35, target: 0.42, max: 0.45 },
    decoratedDensity: { min: 45, target: 58, max: 70 },
...
export function buildAliceGenerationStandard(context: PersonalizationContext = {}) {
  ...
  const personalizationRule = isMilk
    ? "REGRA CAIXA MILK: coloque nome e idade em duas faces laterais alternadas, na parte inferior da area visivel do corpo. Use placa de 30-44% da largura da face e fonte delicada e menor. Personagem ou elemento tematico pode ocupar a parte superior, sem atravessar nem ficar atras do texto. Nunca posicione a personalizacao nas abas superiores, telhado, fundo, cola ou area escondida depois da montagem."
    : "REGRA GLOBAL DE PERSONALIZACAO: reserve a faixa inferior de uma face visivel sem personagem atras ou atravessando o texto; arte menor pode ocupar a parte superior. Use 38-58% da largura da face (alvo 50%) e tipografia proporcional. Em fundo calmo, prefira lettering com halo; em fundo movimentado, use placa solida simples ou a placa propria do tema.";
...
- Personalizacao no terco inferior visivel: 38-58% da largura da face, alvo 50%, texto com contraste forte e sem colisao com personagem. ${nameRule} ${ageRule}
- ${personalizationRule}
...
- Composicao (${weights.composition}): identifique se o perfil e CENARIO ou MODULAR. No cenario, todas as faces uteis recebem arte e a cobertura ativa fica em 45-70%, com a faixa do nome protegida. No modular, os quatro papeis visuais ficam preenchidos e a area calma e intencional, nao uma face esquecida.
- Personalizacao (${weights.personalization}): nome exato, legivel e proporcional, area reservada sem personagem, largura de 38-58% da face, idade com 35-45% do nome e contraste alto. Na Milk, duas faces laterais inferiores usam 30-44%.
